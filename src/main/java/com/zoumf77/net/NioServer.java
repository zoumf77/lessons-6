 ...
 * 同时，建议大家查阅：http://tutorials.jenkov.com/java-nio/index.html
 * jenkov也是大师，他的课程更详细一些，并且在github上有一个模拟http server的源代码
 * 有关NIO的相关基本概念，可以对照起来学习，印象更深刻。
 */
public class NioServer {
	...
	/**
	 * 改进的java nio server的代码中，由于buffer的大小设置的比较小。 我们不再把一个client通过socket
	 * channel多次传给服务器的信息保存在beff中了（因为根本存不下）<br>
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			while (true) {
				...
				// selector.select()方法返回当前有几个channel准备好IO操作了。
				int channelcount = 0;
				if ((channelcount = selector.select(100)) == 0) {
					// ================================================
					// 这里视业务情况，可以做一些然并卵的事情
					// ================================================
					continue;
	...
	/**
	 * 在server socket channel接收到/准备好 一个新的 TCP连接后。 就会向程序返回一个新的socketChannel。<br>
	 * 但是这个新的socket channel并没有在selector“选择器/代理器”中注册， 所以程序还没法通过selector通知这个socket
	 * channel的事件。 于是我们拿到新的socket channel后，要做的第一个事情就是到selector“选择器/代理器”中注册这个
	...
	/**
	 * 这个方法用于读取从客户端传来的信息。 并且观察从客户端过来的socket channel在经过多次传输后，是否完成传输。
	 * 如果传输完成，则返回一个true的标记。
	 * 
	 * @param socketChannel
	 * @throws Exception
	...
	private static void readSocketChannel(SelectionKey readyKey) throws Exception {
		...
		if (URLDecoder.decode(message.toString(), "UTF-8").indexOf("over") != -1) {
			...
			logger.info("端口:" + resoucePort + "客户端发来的完整信息======completeMessage : "
			 ...
			 * ByteBuffer.wrap(URLEncoder.encode("回发处理结果", "UTF-8").getBytes());
			 * clientSocketChannel.write(sendBuffer);
			 */
			String sendMessage = resoucePort.toString() + "回发处理结果";
			...
			logger.info("端口:" + resoucePort + "客户端信息还未接受完，继续接受======message : "
